        ...
        <div class="wrap-right">
            ...
            <div class="center mt-4" >
                <div class="row row-cols-xxl-5 row-cols-xl-4 row-cols-lg-3 row-cols-md-2 row-cols-sm-1 row-cols-1 align">
                    @foreach($favorite_film as $key => $favorite)
                    <article class="item-container">
                        <div class="cover" onclick="redirectToFilm('{{ route('film-info', $favorite->movie->slug) }}')" style="cursor:pointer">
                            <img src="{{ asset('uploads/movie/'.$favorite->movie->image) }}" loading="lazy" alt=""
                            class="body-movie__img">
                            <span class="body-movie__name d-block" >{{$favorite->movie->title}}</span>
