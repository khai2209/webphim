        ...
        <div class="wrap-right">
            ...
            <div class="center mt-4" >
                <div class="row row-cols-xxl-5 row-cols-xl-4 row-cols-lg-3 row-cols-md-2 row-cols-sm-1 row-cols-1 align">
                    @if($favorite->isEmpty())
                        <div class="text-light justify-content-center " style="min-width:40rem;">Hãy thêm nhưng bộ phim bạn thích, để có thể lưu lại và xem sau.</div>
                    @else
                        @foreach($favorite as $key => $value)
                            @if($value->movie && $value->user_id == auth()->user()->id)
